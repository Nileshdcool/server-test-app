  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/getDdeDetails",
        objBody,
        false
      );
      // const response = {
      ...
      //     itr: {
      //       mode: "link",
      //       pdfPath: "/var/www/html/triarc-document/70099998821615790525033",
      //     },
      //     bankDetails: {
      //       collectR: {
      ...
      //       },
      //       salarySlip: [
      //         "/var/www/html/triarc-document/Salary Slip/70099998821615790525033/June/CWC.postman_collection (10).json",
      //         "/var/www/html/triarc-document/Salary Slip/70099998821615790525033/July/AnalyticsFox_API_Contract_Sample (1).docx",
      //         "/var/www/html/triarc-document/Salary Slip/70099998821615790525033/May/DDE.postman_collection.json",
      //       ],
      //     },
      //   },
      //   error: false,
...
export const saveBankDetail =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/saveBankDetails",
...
export const deleteSalarySlips =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/deleteDocumentById",
        objBody,
        false
      );
...
export const sendItrLinkDetails =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/sendITRLinkToCustomer",
        objBody,
        false
      );
      if (!response.data.error) {
...
export const verifyItrCreds =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/verifyITRCredential",
        objBody,
        false
      );
      if (!response.data.error) {
...
export const uploadItrDocuments =
  ...
  async (dispatch) => {
    ...
    try {
      const response = await post(
        "/triarc-sales/dde/uploadITRDocument",
        objBody,
        false,
        true
      );
...
export const deleteITRCreds = (path, appId) => async (dispatch) => {
  ...
  try {
    const response = await get(
      `/triarc-sales/dde/deleteITRDocument?filePath=${path}&applicantUniqueId=${appId}`,
      false
    );
    if (!response.data.error) {
      ts(response.data.message);
...
//getting Fibit Url
export const getFinbitUrl =
  (objBody = {}) =>
  async (dispatch) => {
    dispatch(loading(true));
    try {
      const response = await post(
        "/triarc-sales/dde/getFinbitUrl",
        objBody,
        false
      );
      if (!response.data.error) {
        ts(response.data.message);
        return dispatch(getFinbitUrlSuccess(response.data.data));
      } else {
        te(response.data.message);
        return dispatch(getFinbitUrlFailure(response.data.data));
      }
    } catch (err) {
      dispatch(getFinbitUrlFailure(err));
    } finally {
      dispatch(loading(false));
    }
  };
