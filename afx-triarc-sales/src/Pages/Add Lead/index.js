...
import { getPincodeDetail } from "../../Redux/Services/Qde";
...
const Lead = (props) => {
      ...
      if (leads && leads.leads && leads.leads.data) {
        ...
        if (type === "edit" && data !== "" && leads.fetchLeadDetailsSuccess) {
          if (leads.leads.data) {
            ...
            setCity(leads?.leads?.data?.city);
            setState(leads?.leads?.data?.state);
  ...
  const handleFormChange = (changedFields, allFields) => {
    ...
    // if (
    //   changedFields.customerPincode &&
    //   changedFields.customerPincode.length === 6
    // ) {
    //   console.log("pincode.......", changedFields.customerPincode);
    //   setPincodeDetails(changedFields.customerPincode, form);
    // }
  };
