...
const Summary = (props) => {
  ...
  const selfEmployedFlag =
    props.Summary &&
    props.Summary.loansummary &&
    props.Summary.loansummary.data &&
    props.Summary.loansummary.data.mainapplicant &&
    props.Summary.loansummary.data.mainapplicant.indSelfSoleFlag;

  // setting Branch Name
  useEffect(() => {
    let flag = true;
    console.log(
      "props?.Summary?.loansummary?.data?.branchName",
      props?.Summary?.loansummary?.data?.branchName
    );
    if (props?.Summary?.loansummary?.data?.branchName && flag) {
      // update branch in localStorage
      let userData = localStorage.getItem("UserData");
      let userDataCopy = JSON.parse(userData);
      userDataCopy.branchName =
        props?.Summary?.loansummary?.data?.branchName;
