...
import { Button, Col, Form, Row, Table, Upload, Radio } from "antd";
...
function DDEbank(props, history) {
  ...
  const [finbit, setFinbit] = useState("")
  ...
  function convertDate(d) {
    ...
    return parts && parts[5] + "-" + months[parts[1]] + "-" + parts[2];
  }
  ...
  const onChange = async (e) => {
    const response = props.getFinbitUrl({
      applicantUniqueId: props.match.params.id,  
      type: e
    });
    let res = await response
    if (res !== null) {
      if (e === "uploadBankStatement") {
        window.open(res.payload.finbitLink);
      }
    }
  }
          ...
          <Form.Item name="mode">
            {props.freezeCase || props.freezeUser || props.bureauFreeze ? (
              <Radio.Group
                ...
                <Radio disabled value={"uploadBankStatement"}>
                  Upload Bank Statement
                </Radio>
                <Radio disabled value={"sendLinkToCustomer"}>
              ...
              <Radio.Group
                ...
                style={{ display: "flex", width: "100%" }}
                className="QDe-scheme-radio">
                <Radio value={"uploadBankStatement"}>
                  Upload Bank Statement
                </Radio>
                <Radio value={"sendLinkToCustomer"}>
                  Send link to Customer
                </Radio>
              </Radio.Group>
            )}
