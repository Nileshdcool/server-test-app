...
import { te, ti, tw } from "../../Utility/ReduxToaster";
...
class PersonalDetails extends Component {
  ...
  async componentDidMount() {
    ...
    if (
      ...
      if (this?.props?.qde?.getQdeSectionDetails?.data?.personalDetails) {
        this.form &&
      ...
      if (
        ...
        this.setState({ showAccHolderName: true });
      }
      if (
        this?.props?.qde?.getQdeSectionDetails?.data?.personalDetails?.verified
      ) {
        this.setState({ allowPenny: true });
        this.setState({ verified: true });
      }
      this.setState({ showPenny: true });
      this.setState({
        verified:
          this?.props?.qde?.getQdeSectionDetails?.data?.personalDetails
            ?.verified,
  ...
  async componentDidUpdate(prevProps, prevState) {
    if (
      prevProps.qde.pennyDropDetails !== this.props.qde.pennyDropDetails 
    ) {
      this.form &&
        this.form.setFieldsValue({
          accountHolderName:
      ...
      if (this.props.qde.pennySuccess) {
        this.setState({ verified: true });
        this.setState({ allowPenny: true });
        
      } else {
        this.setState({ verified: false });
        this.setState({ allowPenny: false });
      }
    }
  ...
  submitForm = (e) => {
    ...
    this.props.saveUpdatePersonalDetails({
      ...
      verified: this.state.allowPenny,
      id,
    });
  };
  ...
  // handle penny drop api
  handlePennyDrop = () => {
    // this.setState({ allowPenny: true });
    if (this.state.ifscCode) {
      this.props.getPennyDetails({
        applicantUniqueId: this.props.match.params.id,
  ...
  handleFunctn = async (changedFields, allFields) => {
    ...
    if (allFields.accountNumber) {
      this.setState({ bankNumber: allFields.accountNumber });
    }

    if (changedFields.ifscNumber || changedFields.accountNumber) {
      this.setState({ allowPenny: false });
      this.setState({ showPenny: true });
      this.setState({ verified: false });
      this.setState({ showAccHolderName: false });
    }

    if (allFields.ifscNumber) {
      this.setState({ ifscCode: allFields.ifscNumber });
    }
  ...
  render() {
    ...
    console.log(this.state.allowPenny, this.state.showPenny, this.state.verified);
          ...
          <Form
                ...
                <div className={"mui-dropdown-wrapper"}>
                  ...
                  <Form.Item
                    ...
                    <TextField
                      ...
                      style={{
                        pointerEvents: this.state.verified ? "none" : "auto",
                      }}
                      label="Account Type*"
                      fullWidth
                      SelectProps={{
              ...
              <Col lg={8}>
                <Form.Item
                  ...
                  <TextField
                    inputProps={inputProps}
                    style={{
                      pointerEvents: this.state.verified ? "none" : "initial",
                    }}
                    disabled={this.state.verified}
                    InputLabelProps={{
                      shrink:
            ...
            <Row className={"typeRow qde-loandetails-picker"} gutter={(40, 40)}>
              ...
              <Col lg={8}>
                {
                  // this.props?.qde?.pennyDropDetails?.accountHolderName &&
                  (this.state.allowPenny || this.state.showAccHolderName) && (
                    <Form.Item
                      ...
                      <TextField
                        ...
                        // }
                        style={{
                          pointerEvents: this.state.verified
                            ? "none"
                            : "initial",
                        }}
                        inputProps={inputProps}
                        ...
                        InputLabelProps={{
                          shrink:
                            this.form &&
                            this.form.getFieldValue("accountHolderName"),
                        }}
                        fullWidth={true}
