...
class DetailsUtility extends Component {
  state = {
    ...
    utilityBillType: "",
    verified1: null,
    InputLabelPropsAddr1: {shrink: true},
  ...
  uploadPOA = async (changedFields, allFields) => {
    if (allFields.address1) {
      this.setState({ InputLabelPropsAddr1: { shrink: true } });
      this.setState({ InputLabelPropsAddr2: { shrink: true } });
    } else {
      this.setState({ InputLabelPropsAddr1: { shrink: false } });
  ...
  setUtilityData = () => {
    ...
    const payload = {
      ...
      landmark1: utilityDetails.landmark1,
      landmark2: utilityDetails.landmark2,
    };
    ...
    this.form && this.form.setFieldsValue(payload);
    this.setState({
      ...this.state,
      ...statePayload,
    });
        ...
        isEmpty(
          this.props.qde.getQdeSectionDetails &&
            ...
            this.props.qde.getQdeSectionDetails.data.additionalDetails
              .utilityDetails &&
            this.props.qde.getQdeSectionDetails.data.additionalDetails
              .utilityDetails.address1
        )
      ) {
        this.setState({ InputLabelPropsAddr1: await { shrink: false } });
        this.setState({ InputLabelPropsAddr2: await { shrink: false } });
      } else {
        this.setState({ InputLabelPropsAddr1: await { shrink: true } });
    ...
    if (
      ...
      if (this.state.POADoc === "lbill") {
        const landLineAddress =
          ...
          let splitAddress = landLineAddress.split(" ");
          let halfSplit = Math.floor(splitAddress.length / 2);
  ...
  render() {
    ...
    console.log(
      "this.props.qde-->",
      this?.props?.qde?.getQdeSectionDetails?.data?.additionalDetails?.bureauStatus
    );
    return (
      <div className="additionalDetails-Container add-POA">
        ...
        >
          ...
          {this.props.selfEmployedJourney ? (
            <p className="sub-title">Registered Address 2</p>
          ) : (
            ...
            <Col lg={8}>
              ...
              >
                <TextField
                  ...
                  // InputLabelProps={this.state.InputLabelPropsAddr1}
                  fullWidth={true}
                  id="standard-basic"
          ...
          <div className="buttonSave  mt-4 mb-2">
            {!(this.props.freezeCase || this.props.freezeUser) && (
              <Button
                ...
                htmlType={"submit"}
                disabled={
                  (!this.props.qde.utilityDocUploadSuccess && !billType) ||
                  this.props.bureauFreeze
                  // ||
                  // (!this?.props?.qde?.getQdeSectionDetails?.data
                  //   ?.additionalDetails?.bureauStatus)
                }
              >
                Save
              </Button>
