...
import { getReasonList } from "../../Utility/Services/LeadList";
...
class Success extends React.Component {
  ...
  componentDidMount() {
    ...
    (async () => {
      const reason = await getReasonList();
      console.log("res", reason);
      this.setState({ reason: reason?.data });
    })();
  }
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    console.log("ee", this.state?.reasonValue);
    if (prevState.reasonValue !== this.state.reasonValue) {
    }
    if (prevState.comment !== this.state.comment || prevState.reasonValue !== this.state.reasonValue) {
      
      if(this.props?.Summary?.loansummary?.data?.mainapplicant?.reasonDropDownFlag){
        if(!_?.isEmpty( this.state.reasonValue) && !_?.isEmpty( this.state.comment)){
          console.log("ee horray 1")
         ...
         if (
           ...
           !_?.isEmpty(this.state.comment)
         ) {
           console.log("ee horray");
           this.setState({ disableBtn: false });
         }
      }
    }
  ...
  render() {
    ...
    const creditButtonFlag =
      this.props?.Summary?.loansummary?.data?.mainapplicant?.creditButtonFlag;
      
    const freezeUser =
      this.props?.Summary?.loansummary?.data?.modelAccess[0]?.read;

    console.log("this.props.qde-->", this.props.qde);

    const emi =
      this.props.qde &&
      this.props.qde.getLoanData &&
      this.props.qde.getLoanData.data &&
      this.props.qde.getLoanData.data.emiAmount &&
      this.props.qde.getLoanData.data.emiAmount.toFixed(2);

    const getReasonDropdown = [
      <option hidden> {} </option>,
      //  ...map(bankType, (item) => {
      //    return (
      //      <option value={item.bankAccountType}>
      //        {item.bankAccountType}
      //      </option>
      //  );
      //  }),
      this.state?.reason?.map((re) => (
        <option value={re?.reason}>{re?.reason}</option>
      )),
    ];
        ...
        <div className="mainContainerSuccess">
          ...
          <div className={"Successcontainer"}>
            ...
            <div className="buttonSuccessPage">
              ...
              {this.props.match.params.journey === "applicant" &&
                this.props.match.params.offerType !== "final" &&
                (creditButtonFlag || !(freezeCase || freezeUser)) && (
            ...
            >
              ...
              {this.props?.Summary?.loansummary?.data?.mainapplicant?.reasonDropDownFlag && (
