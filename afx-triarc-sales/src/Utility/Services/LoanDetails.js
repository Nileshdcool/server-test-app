...
export const loanDetails = async (objBody = {}, type) => {
  try {
    const response = await post(`/triarc-sales/qde/${type}`, objBody, false).then(
      (res) => {
        return res.data;
      }
    );
...
export const getSCD = (objBody = {}) => {
  return post("/triarc-sales/master/getSchemeDetailsBySchemeCode", objBody).then(
    (res) => {
      console.log("res...",res)
      return res;
    }
