...
function deviation(props) {
  ...
  let applicantId = props.location;

  const [loader, setLoader] = useState(false);
  const { setHeading, setIsTimerVisible, start, reset } = useContext(
    App.CaseContext
  );
  const [data, setData] = React.useState({});
  const [devType, setDevType] = useState(applicantId.state?.from);
          ...
          <Col lg={24}>
            <Row type={"flex"} justify={"center"}>
              ...
              <Button
                ...
                // disabled = {data?.breFinalResult == "Manual Underwriting"}
              >
                {" "}
                Next{" "}   
              </Button>
